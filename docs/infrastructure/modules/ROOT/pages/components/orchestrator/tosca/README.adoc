...
----
 primary_hpc:
  ...
  ckan_api_key:
    default:
      REPLACE: ckan_input_resource.storage.config.key
...
To begin with, every _node_ is identified by its name (`hpc_wm` in the example below), and a type is assigned to it.
In the case of {uri-mso4sc}[MSO4SC] each _node_ represents either a workload manager infrastructure (e.g Slurm on an HPC), or a job. See {uri-cloudify-hpc-types}[Cloudify HPC plugin types] for more information and examples.

.Workload Manger node example
[source,yaml]
----
node_templates:
    hpc_wm:
        ...
        properties:
            ...
            workdir_prefix: "example"
----

The example above represents a workload manager. All HPC infrastructures must have defined its workload manager. Then, it is configured using the inputs (using fuction `get_input`). Detailed information about how to configure the workload manager is in the {uri-cloudify-hpc-config}[Cloudify HPC plugin configuraton].
...
Finally, jobs have two types of relationships: __job_managed_by_wm__, to stablish in which workload manager the job is going to run, and __job_depends_on__, to describe the dependency between jobs. In the example above, `fourth_job` depends on `three_parallel_job` and `second_parallel_job`, so it will not execute until the other two have finished. In the same way, `three_parallel_job` and `second_parallel_job` depends on `first_job`, so they will run in parallel once the first job is finished. All jobs are contained in `hpc_wm`, so they will run on that workload manager using the credentials provided. See {uri-cloudify-hpc-relationships}[Cloudify HPC plugin relationships] for more information.
